<?php

return [
    'accept_invitation' => 'Accept the invitation',
    'actions' => 'Actions',
    'address' => 'Address',
    'address_cannot_be_geocoded' => 'Your address cannot be geolocalized. Please use a simple format, like "number street, city"',
    'address_privacy_and_help' => 'How to fill out the address field? Only fill in the information that you are willing to put online. The address information, even partial, allows us to display a map with the location of people, groups and actions. If you do not want to be shown on this map do not put anything here. You can also just put your town (or your street name without the house number) to appear on the map, but not in a precise manner.',
    'admin' => 'Administrator',
    'administration' => 'Administration',
    'admins' => 'Admins',
    'admin_of' => 'Admin of',
    'agenda' => 'Calendar',
    'agenda_my' => 'My group\'s calendars',
    'agenda_others' => 'Other groups\' calendars',
    'all_groups' => 'All groups',
    'all_is_read' => 'You\'ve read everything in this discussion',
    'appearance' => 'Appearance',
    'archive' => 'Archive',
    'are_you_sure' => 'Are you sure?',
    'attach_file_help' => 'Simply drop a file on the editor to add it or use the image button in the toolbar',
    'attend' => 'Attend',
    'attend_to' => 'Attend to',
    'author' => 'Author',
    'a_message_for_you' => 'A message for you',
    'back_to' => 'Back to',
    'because_you_asked_for_it' => 'Because you asked for it',
    'begins' => 'Begins',
    'body' => 'Body',
    'cancel' => 'Cancel',
    'candidates' => 'Candidates',
    'changed' => 'Changed',
    'changed_from' => 'Changed from',
    'changed_to' => 'Changed to',
    'change_my_password' => 'Change my password',
    'choose_a_group' => 'Choose a group',
    'click_here' => 'Click here',
    'click_here_to_change_your_password' => 'Click here to change your password',
    'click_to_confirm' => 'Click to confirm',
    'comments' => 'Comments',
    'confirm_password' => 'Confirm password',
    'confirm_user' => 'Confirm user',
    'confirm_your_email' => 'Confirm your email',
    'contact' => 'contact',
    'contact_this_user' => 'Contact this user',
    'county_help' => 'Filling this field might help the geocoder to find the correct location.',
    'cover_added_error' => 'Error adding a cover, please check file type (jpeg or png) and image size.',
    'cover_added_successfully' => 'Cover added successfully.',
    'create' => 'Create',
    'created' => 'Created',
    'create_action' => 'Add an event',
    'create_discussion' => 'Start a new discussion',
    'create_files_button' => 'Upload file(s)',
    'create_file_button' => 'Upload file',
    'create_folder' => 'Create a folder',
    'create_folder_button' => 'Create a folder',
    'create_folder_help' => 'Enter the name of the folder',
    'create_link' => 'Create a link',
    'create_link_button' => 'Create Link',
    'create_link_help' => 'Here you can add a link (URL). For example link to videos, shared documents, etc...
  Enter a correct URL starting with http:// and set a title for this link. You can also add tags.',
    'custom_content' => 'Custom content',
    'delete' => 'Delete',
    'deleteGroup' => 'Delete Group',
    'delete_confirm_button' => 'Delete',
    'delete_confirm_title' => 'Are you sure you want to delete?',
    'description' => 'Description',
    'details' => 'Details',
    'discussions' => 'Discussions',
    'dont_reply_to_this_email' => 'Don\'t reply to this email, use the reply button(s) instead',
    'download' => 'Download',
    'download_ical' => 'Download the calendar in iCal format',
    'do_nothing' => 'Do nothing',
    'duplicate' => 'Duplicate',
    'edit' => 'Edit',
    'edit_item' => 'Edit item',
    'edit_my_profile' => 'Edit my profile',
    'email' => 'Email',
    'email_not_verified' => 'Email not verified',
    'email_not_verified_send_again_verification' => 'Click here to send your verification email again',
    'email_verified' => 'Email verified',
    'embed' => 'Embed this page',
    'ends' => 'Ends',
    'features' => 'Features',
    'filename' => 'Filename',
    'files' => 'Files',
    'files_in_this_group' => 'Files in this group',
    'filter' => 'Filter',
    'folder' => 'Folder',
    'forgotten_password' => 'Forgotten password',
    'grid' => 'Grid',
    'group' => 'Group',
    'groups' => 'Groups',
    'group_admins' => 'Group admins',
    'group_admin_users' => 'Contact person(s)',
    'group_home' => 'Home',
    'group_tags_help' => 'For your information, here are all the tags in use in this group',
    'hello' => 'Hello',
    'help' => 'Help',
    'here_are_the_latest_news_of' => 'Here is the latest news from',
    'here_is_the_description_of_the_group' => 'Here is the description of the group',
    'history' => 'history',
    'howdy' => 'Whoops!',
    'how_does_it_work' => 'How does it work?',
    'if_you_already_have_account' => 'If you already have an account',
    'if_you_didnt_ask' => 'If you didn\'t ask to join',
    'if_you_dont_have_account' => 'If you don\'t have an account (yet!)',
    'if_you_dont_want_news_anymore' => 'If you don\'t want news anymore',
    'if_you_dont_want_to_change_password_do_nothing' => 'If you don\'t want to change your password, do nothing',
    'if_you_donwt_want_to_join_do_nothing' => 'If you don\'t want to join, do nothing. You will not be asked again to join the group',
    'if_you_want_participate_in_this_group' => 'If you want to participate in this group',
    'in' => 'in',
    'inbox_help' => 'The group email can only be used by group members',
    'inside' => 'inside',
    'insights' => 'Insights',
    'invitation_sent_again' => 'Invitation sent again',
    'invitation_to_join' => 'Invitation to join',
    'invite_already_used' => 'Invitation already used',
    'invite_not_found' => 'Invitation not found',
    'in_the_discussion' => 'in the discussion',
    'join' => 'Join',
    'join_a_group' => 'Please join a group or create a group',
    'join_the_group' => 'Join the group',
    'join_this_group' => 'Join this group',
    'last_activity' => 'Last activity',
    'latest_discussions' => 'Latest discussions',
    'latest_discussions_my' => 'Latest discussions in my groups',
    'latest_discussions_others' => 'Latest discussions in other groups',
    'latest_files' => 'Latest files',
    'latest_users' => 'Latest users',
    'leave_this_group' => 'Leave this group',
    'leaving_help_message' => 'We are sad to see you go... confirm that you want to leave this group. You won\'t receive notifications anymore.',
    'leaving_the_group_called' => 'Leaving the group called',
    'link' => 'Link',
    'list' => 'List',
    'listed_location_help' => 'Will this event take place in one of these locations?',
    'location' => 'Location',
    'location_cannot_be_geocoded' => 'Location cannot be geocoded',
    'location_name_help' => 'Enter a name for this location for future re-use.',
    'location_other' => 'Otherwise please enter your venue here:',
    'location_privacy_and_help' => 'Only fill in the information that you are willing to put online. The address information, even partial, allows us to display a map with the location of people, groups and actions. If you do not want to be shown on a map do not put anything here. You can also just put your town (or your street name without the house number) to appear on the map, but not in a precise manner.',
    'login' => 'Login',
    'logout' => 'Logout',
    'logs' => 'Logs',
    'made_with' => 'Made with',
    'mail' => 'Email',
    'make_user_admin' => 'Make user admin',
    'map' => 'Map',
    'map_info' => 'If you complete your address in your user profile, you will appear here',
    'max_file_size' => 'Maximum file size',
    'members' => 'Members',
    'member_since' => 'Member since',
    'message' => 'message',
    'message_sent' => 'Message sent',
    'modify' => 'Modify',
    'more' => 'More',
    'my_groups' => 'My groups',
    'name' => 'Name',
    'navigation' => 'Navigation',
    'new' => 'New',
    'news_from_group_email_subject' => 'News from the group',
    'new_tags' => 'Add new tags',
    'next_actions' => 'Next actions',
    'no' => 'no',
    'nothing_yet' => 'There is nothing here...yet!',
    'notifications' => 'Notifications',
    'notifications_interval' => 'Notifications',
    'notified' => 'Notified',
    'not_allowed' => 'Not allowed',
    'not_a_group_admin' => 'You are not an admin of this group',
    'not_a_member' => 'You are not a member of this group',
    'not_logged_in' => 'You are not logged in',
    'no_file_selected' => 'No file selected',
    'or' => 'or',
    'other' => 'Other',
    'overview' => 'Overview',
    'password' => 'Password',
    'password_reset' => 'Password reset',
    'permissions' => 'Permissions',
    'personal_ical_group_link' => 'Private iCal link (do not share)',
    'photo' => 'Photo',
    'pin' => 'Pin',
    'preferences_for' => 'Preferences for',
    'presentation' => 'Presentation',
    'private' => 'Private',
    'profile' => 'Profile',
    'public' => 'Public',
    'public_ical_group_link' => 'Public iCal link',
    'recent_activity' => 'Recent activity',
    'recover_content' => 'Recover deleted content',
    'regard' => 'Regards',
    'register' => 'Register',
    'registered' => 'Registered',
    'registration_time' => 'Registration time',
    'remember_me' => 'Remember me',
    'remove_user' => 'Remove User',
    'remove_user_admin' => 'Revoke admin rights',
    'reply' => 'Reply',
    'reset_password_email_subject' => 'Reset your password',
    'ressource_created_successfully' => 'Resource created successfully',
    'ressource_deleted_successfully' => 'Resource deleted successfully',
    'ressource_geocoded_successfully' => 'Resource geocoded successfully',
    'ressource_not_created_successfully' => 'Resource Not Created Successfully',
    'ressource_not_updated_successfully' => 'Resource Not Updated Successfully',
    'ressource_updated_successfully' => 'Resource updated successfully',
    'root' => 'Root',
    'rss' => 'RSS Feed',
    'save' => 'Save',
    'search' => 'Search',
    'select_one_file' => 'Select one file',
    'select_one_or_more_files' => 'Select one or more files',
    'send' => 'send',
    'send_my_recover_email' => 'Send my recover email',
    'sent_you_a_message' => 'sent you a message',
    'server_administration' => 'Server administration',
    'settings' => 'Settings',
    'show_all' => 'Show all',
    'show_history' => 'Show history',
    'show_list' => 'Show file list',
    'something_wrong' => 'Something is wrong with your submission',
    'sort' => 'Sort',
    'sort_by_created_at_asc' => 'Sort by creation date ascending',
    'sort_by_created_at_desc' => 'Sort by creation date descending',
    'sort_by_filesize_asc' => 'Sort by filesize ascending',
    'sort_by_filesize_desc' => 'Sort by filesize descending',
    'sort_by_name_asc' => 'Sort by name ascending',
    'sort_by_name_desc' => 'Sort by name descending',
    'start' => 'Start',
    'started_by' => 'Started by',
    'start_date' => 'Start date',
    'start_time' => 'Start time',
    'status' => 'Status',
    'stop' => 'Stop',
    'stop_date' => 'Stop date',
    'stop_time' => 'Stop time',
    'success' => 'Success',
    'tags' => 'Tags',
    'tags_help' => 'Choose or type one or more tag (keywords if you prefer). This will help others to find your contribution. In the list you will see the tags already used elsewhere. Try to follow the existing tags.',
    'thank_you' => 'Thank you for using this application',
    'thank_you_for_joining' => 'Thank you for joining',
    'thinks_that_you_might_want_to_join' => 'thinks that you might want to join',
    'this_will_allow_you_to_be_informed' => 'this will allow you to be informed and to participate',
    'title' => 'Title',
    'unarchive' => 'Unarchive',
    'unattend' => 'Unattend',
    'unpin' => 'Unpin',
    'unread' => 'unread',
    'unread_discussions' => 'Unread discussions',
    'upload' => 'Upload',
    'upload_file' => 'Upload File',
    'upload_files' => 'Upload Files',
    'upload_new_file_version' => 'Upload a new version of this file',
    'username' => 'username',
    'users' => 'Users',
    'users_list' => 'Users',
    'user_added_successfuly' => 'User added successfully',
    'user_applied_to_the_group' => 'User :user applied to the group :group',
    'user_attending' => 'Users attending',
    'user_invited' => 'User invited',
    'user_made_member_successfuly' => 'User made member successfully',
    'user_not_attending' => 'Users not attending',
    'user_profile' => 'User profile',
    'user_removed_successfuly' => 'User Removed Successfuly',
    'visibility' => 'Visibility',
    'visibility_help' => 'You can make this element public even if your group is private. In this case everyone will be able to see this element without being member.',
    'visit' => 'Visit',
    'visit_link' => 'Visit Link',
    'we_need_to_confirm' => 'We need to confirm your account',
    'when' => 'when',
    'where' => 'where',
    'yes' => 'yes',
    'your_groups' => 'Your groups',
    'your_search_for' => 'Your search for',
    'you_are_not_an_admin' => 'You are not an admin',
    'you_are_now_a_member_of_this_group' => 'You are now a member of this group',
    'you_asked_for_assword_change' => 'You asked for a password change',
    'you_can_create_one_here' => 'You can create one here',
    'you_can_login_here' => 'You can login here',
    'you_dont_have_an_account_create_one_now' => 'You don\'t have an account, create one now',
    'you_have_been_added_to_the_group' => 'You have been added to the group',
    'you_have_been_mentionned_by' => 'You have been mentioned by',
    'you_have_verified_your_email' => 'You have verified your email address',
    'you_login' => 'login here',
    'you_receive_this_email_from_the_group' => 'You receive this email from the group',
    'you_register' => 'register here',
];
