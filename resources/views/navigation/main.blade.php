    ...
    <div class="navigation-container">
        ...
        <form up-target="body" class="form-inline my-2" role="search" action="{{ url('search') }}" method="get">
            <div class="input-group">
                <input class="form-control" type="text" name="query" placeholder="{{ trans('messages.search') }}..."
                    aria-label="Search">
        ...
        <div class="navigation">
            ...
            @auth
                ...
                <a up-target="body" class="item" href="{{ route('users.show', Auth::user()) }}"><i
                        class="fa fa-btn fa-user"></i> {{ trans('messages.profile') }}</a>
                <a up-target="body" class="item" href="{{ route('users.edit', Auth::user()) }}"><i
            ...
            @if (\Config::has('app.locales'))
                ...
                @foreach (\Config::get('app.locales') as $locale)
                    @if ($locale !== app()->getLocale())
                        <a up-target="body" class="item" href="{{ route('index') }}?force_locale={{ $locale }}">
                            {{ strtoupper($locale) }}
                        </a>
                    @endif
                @endforeach
