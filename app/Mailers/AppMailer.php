    ...
    */
    ...
    public function sendNotificationEmail(Group $group, User $user)
    {
        ...
        if ($user->verified == 1)
        {
            ...
            $events = \App\CalendarEvent::where('start', '>', Carbon::now())
            ->where('stop', '<', Carbon::now()->addWeek()->addWeek())
            ->where('group_id', "=", $group->id)
            ->orderBy('start')
            ->get();

            // we only trigger mail sending if a new event has been **created** since last notfication email.
            ...
            if (count($discussions) > 0 or count($files) > 0  or ($events_count) > 0)
            {
                ...
                'files' => $files, 'users' => $users, 'calendarevents' => $events, 'last_notification' => $last_notification], function ($message) use($user, $group) {
                    $message->from(env('MAIL_NOREPLY', 'noreply@example.com'), env('APP_NAME', 'Laravel'))
                    ->to($user->email)
