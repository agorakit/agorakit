...
use Agorakit\Group;
use Agorakit\Mail\Notification;
use Agorakit\User;
use Auth;
use Carbon\Carbon;
use DB;
...
class SendNotifications extends Command
{
        ...
        if (is_array($notifications)) {
            if (count($notifications) > 0) {
                foreach ($notifications as $notification) {
                    $user = \Agorakit\User::find($notification->user_id);
                    $group = \Agorakit\Group::find($notification->group_id);
    ...
    public function getNotificationsToSend()
    {
        ...
        order by rand()
        limit :batch
        ', ['now' => Carbon::now(), 'membership' => \Agorakit\Membership::MEMBER, 'batch' => $this->option('batch')]);
    ...
    public function sendNotificationEmail(Group $group, User $user)
    {
        ...
        $membership = \Agorakit\Membership::where('user_id', '=', $user->id)
            ...
            ->where('membership', '>=', \Agorakit\Membership::MEMBER)
            ->first();
        ...
        if ($membership) {
            ...
            $files = \Agorakit\File::where('created_at', '>', $membership->notified_at)
                ->where('group_id', '=', $group->id)
                ->get();
            ...
            $actions = \Agorakit\Action::where('start', '>', Carbon::now()->toDateTimeString())
            ...
            $actions_count = \Agorakit\Action::where('created_at', '>', $membership->notified_at)
                ->where('group_id', '=', $group->id)
                ->count();
    ...
    {
        $discussions = \Agorakit\Discussion::fromQuery('select * from
        (
            select *,
    ...
    {
        $users = \Agorakit\User::fromQuery('select * from users where id in
            ...
            ', ['user_id' => $user_id, 'group_id' => $group_id, 'since' => $since, 'membership' => \Agorakit\Membership::MEMBER]);
